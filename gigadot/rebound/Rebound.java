...
public class Rebound {
     ...
     * @param packagePrefix
     */
    public Rebound(Logger logger,String packagePrefix) {
        this(logger,packagePrefix, false);
    }
     ...
     */
    public Rebound(Logger logger,String packagePrefix, boolean includeAbstractAndInteface) {
        Rebound.logger = logger;
    ...
    public Rebound(Logger logger,String packagePrefix, boolean includeAbstract, boolean includeInteface) {
        Rebound.logger = logger;
        this.packagePrefix = packagePrefix == null ? "" : packagePrefix;
    ...
    public <T> Set<Class<? extends T>> getSubClassesOf(final Class<T> clazz) {
        ...
        if (packagePrefix.isEmpty()) {
            if (logging) {
                logger.warning("Package prefix is empty. Rebound will look for subclasses inside "
        ...
        while (allEntries.hasMoreElements()) {
            ...
            if (!jarEntry.isDirectory() && entryName.startsWith(packagePathFilter) && entryName.endsWith(".class")) {
                try {
                    ...
                    if (logging) {
                        logger.log(Level.SEVERE, "Cannot get Class for name : " + jarEntry.getName(), ex);
                    }
                }
            }
        }
    ...
    private static File toFile(URL url) {
        try {
            ...
            if (logging) {
                logger.log(Level.SEVERE, "Unable to decode jar file location using UTF-8.", ex);
            }
        }
        if (logging) {
            logger.warning("URL is not decoded : " + url.toString());
        }
    ...
    private static JarFile toJarFile(final URL url) {
        try {
            ...
            try {
                ...
                if (logging) {
                    logger.log(Level.SEVERE, "Cannot create JarFile object for path : " + jarPath, ex);
                }
            }
        } catch (UnsupportedEncodingException ex) {
            if (logging) {
    ...
    private static URL toURL(File file) {
        try {
            ...
            if (logging) {
                logger.log(Level.SEVERE, "Cannot create JarFile object for path : " + file.getPath(), ex);
            }
        }
        if (logging) {
            logger.info("Null JarFile is returned.");
